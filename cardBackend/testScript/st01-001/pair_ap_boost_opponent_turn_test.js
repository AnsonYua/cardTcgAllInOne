...
const SCENARIO_PATH = 'ST01-001/pair_ap_boost_opponent_turn';
...
async function main() {
    ...
    await requestJson('POST', `${BASE_URL}/api/game/player/endTurn`, {
        gameId,
        playerId: PLAYER_A
    });
    ...
    if (gameEnv.currentPlayer !== PLAYER_B) {
        throw new Error(`Expected currentPlayer to be ${PLAYER_B} after endTurn, got ${gameEnv.currentPlayer}`);
    }

    const playerAUnits = collectUnits(gameEnv, PLAYER_A);
    const playerBUnits = collectUnits(gameEnv, PLAYER_B);

    assertPairApBoost(playerAUnits, 0, 'Player A units');
    assertPairApBoost(playerBUnits, 0, 'Player B units');

    console.log('✅ Pair AP boost only on your turn: PASS');
}

main().catch((error) => {
    console.error('❌ Pair AP boost only on your turn: FAIL');
