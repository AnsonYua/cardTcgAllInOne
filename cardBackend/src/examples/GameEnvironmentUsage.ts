...
export function exampleCreateNewGame(): GameEnvironment {
    ...
    player1.deck = new PlayerDeckDataResp(
        0, // currentLeaderIdx
    ...
    player2.deck = new PlayerDeckDataResp(
        0, // currentLeaderIdx
    ...
    console.log('Game summary:', GameEnvironmentHelper.getGameSummary(gameEnv));
...
export function exampleConvertLegacyJSON(): GameEnvironment {
    console.log('\\n=== Example 2: Converting Legacy JSON ===');
    ...
    const validation = GameEnvironmentValidator.validateLegacyJSON(legacyGameEnv);
    if (!validation.isValid) {
        console.error('Legacy JSON validation failed:', validation.errors);
        return createGameEnvironment();
    }
    
    // Convert to GameEnvironment class
    const gameEnv = GameEnvironmentAdapter.fromLegacyJSON(legacyGameEnv);
    ...
    console.log('Zone occupied (legacy_player_1, TOP):', gameEnv.zones.isZoneOccupied('legacy_player_1', ZoneType.TOP));
...
export function exampleGameOperations(): void {
    ...
    gameEnv.playCard('player_2', 'c-7_uid_7', ZoneType.HELP, true);
    console.log('Player 2 played card face-down in HELP zone');
    
    // Check zone status
    console.log('\\nZone Status:');
    console.log('Player 1 TOP occupied:', gameEnv.zones.isZoneOccupied('player_1', ZoneType.TOP));
    console.log('Player 1 LEFT occupied:', gameEnv.zones.isZoneOccupied('player_1', ZoneType.LEFT));
    console.log('Player 2 HELP occupied:', gameEnv.zones.isZoneOccupied('player_2', ZoneType.HELP));
...
export function exampleSerializationAndValidation(): void {
    ...
    const legacyJSON = GameEnvironmentAdapter.toLegacyJSON(gameEnv);
    ...
    const gameEnv2 = GameEnvironmentAdapter.fromLegacyJSON(legacyJSON);
    console.log('Converted back to GameEnvironment');
    
    // Compare the two instances
    const differences = GameEnvironmentHelper.compare(gameEnv, gameEnv2);
    ...
    const clonedGameEnv = GameEnvironmentHelper.deepClone(gameEnv);
    console.log('Created deep clone');
    
    // Modify clone and compare
    clonedGameEnv.playCard('player_2', 'c-7_uid_7', ZoneType.LEFT);
    const cloneDifferences = GameEnvironmentHelper.compare(gameEnv, clonedGameEnv);
    console.log('Differences after modifying clone:', cloneDifferences);
}

// ============ EXAMPLE 5: Integration with Existing Code ============

export function exampleIntegrationWithExistingCode(): void {
    console.log('\\n=== Example 5: Integration with Existing Code ===');
    ...
    const updatedLegacyGameEnv = GameEnvironmentAdapter.toLegacyJSON(gameEnv);
