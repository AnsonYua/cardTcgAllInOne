...
export class DeployEffectOrderManager {
    static executeOptionChoice(event: OptionChoiceEvent, gameEnv: GameEnvironment): ExecutionResult {
        ...
        return this.resolveAndQueueDeployEvent({
            playerId: event.playerId,
            sourceCarduid: event.data.sourceCarduid,
            selectedOptionIndex: event.data.selectedOptionIndex,
            context: event.data.context
        }, gameEnv, { requireDeployCarduid: false });
    }
    ...
    static executePromptChoice(event: PromptChoiceEvent, gameEnv: GameEnvironment): ExecutionResult {
        ...
        return this.resolveAndQueueDeployEvent({
            ...
            selectedOptionIndex: event.data.selectedOptionIndex,
            context: event.data.context
        }, gameEnv, { requireDeployCarduid: true });
    }

    private static resolveAndQueueDeployEvent(
        args: {
            playerId: string;
        ...
        opts: { requireDeployCarduid: boolean }
    ): ExecutionResult {
        const context = args.context as Partial<DeployEffectOrderContext> | undefined;
        ...
        const selectedIndex = args.selectedOptionIndex;
        ...
        if (opts.requireDeployCarduid && !deployCarduid) {
