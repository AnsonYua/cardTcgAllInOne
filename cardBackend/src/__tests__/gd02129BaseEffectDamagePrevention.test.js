...
describe('GD02-129 deploy effect damage prevention', () => {
    ...
    test('prevents enemy damageShield to base after deploy effect', () => {
        const gameEnv = new GameEnvironment();
        gameEnv.addPlayer('playerId_1', 'P1');
        gameEnv.addPlayer('playerId_2', 'P2');
        gameEnv.currentTurn = 1;
        ...
        const baseUid = 'GD02-129_base_0002';
        expect(PlayerCardManager.placeCardWithEventData(gameEnv, 'playerId_1', {
            carduid: baseUid,
            playAs: 'base'
        }).success).toBe(true);
        ...
        const player = gameEnv.getPlayer('playerId_1');
        const shieldCardData = CardDatabaseManager.getCardDetails('ST01-001');
        const shieldUid = 'ST01-001_shield_0002';
        player.zones.shieldArea.push(createZoneCard(shieldUid, 'ST01-001', shieldCardData, 'playerId_1'));
        ...
        const damageShieldEffect = {
            effectId: 'test_damage_shield',
            action: 'damageShield',
            parameters: { value: 2 }
        };

        const enemyDamageShield = EffectExecutor.applyEffectToTargets(
            gameEnv,
            damageShieldEffect,
            [{ playerId: 'playerId_1' }],
            'playerId_2',
            'ST02-001_enemy_source_0001'
        );

        expect(enemyDamageShield.success).toBe(true);
        expect(player.zones.base[0].damageReceived || 0).toBe(0);
