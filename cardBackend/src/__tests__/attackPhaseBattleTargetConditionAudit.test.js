...
function collectAttackPhaseRulesWithBattleTargetTypeCondition() {
    const rows = [];
        ...
        for (const [cardId, card] of Object.entries(data?.cards || {})) {
            ...
            for (const rule of rules) {
                ...
                const hasAttackTargetType = conditions.some((condition) =>
                    ...
                    condition.type === 'attackTargetCardType' &&
                    condition.scope === 'battle'
                );
                if (!hasAttackTargetType) continue;
                rows.push({
                    file,
                    cardId,
                    cardName: card?.name || cardId,
                    effectId: rule?.effectId || 'unknown_effect',
                    action: rule?.action || 'unknown_action'
                });
            }
        }
    }

    return rows.sort((a, b) => a.cardId.localeCompare(b.cardId) || a.effectId.localeCompare(b.effectId));
}

describe('attack-phase battle target condition audit across listed card data files', () => {
    test('detects rules exposed to pre-battle attackTargetCardType condition timing issues', () => {
        const rows = collectAttackPhaseRulesWithBattleTargetTypeCondition();
